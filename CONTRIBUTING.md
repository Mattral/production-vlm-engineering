...
2. Register it in `_EXAMPLES` in `src/production_vlm/cli.py`.
...
5. Add tests under `tests/` for any new shared library code in `src/production_vlm/`.

## Code style

- `ruff` for linting/formatting (config in `pyproject.toml`); pre-commit hooks are provided
  in `.pre-commit-config.yaml`.
- Keep `src/production_vlm`'s core import surface CPU-only and dependency-light (numpy/scipy/
