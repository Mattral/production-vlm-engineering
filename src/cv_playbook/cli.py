"""Unified CLI entry point: `cv-playbook <command>` (also runnable as `python -m cv_playbook.cli`).
...
def cmd_run_example(args: argparse.Namespace) -> int:
    if args.name not in _EXAMPLES:
        print(f"Unknown example '{args.name}'. Run `cv-playbook list-examples`.", file=sys.stderr)
        return 1
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="cv-playbook", description="Frontier CV Engineering Playbook CLI")
