...
Each example prints a clear table showing whether it ran the real GPU path or the CPU smoke-test fallback, then writes `outputs/<example_name>/results.json`. The benchmark runner collects all five `results.json` files into a single Markdown report.
...
- [Architecture overview](architecture.md) — how the five examples and shared library fit together
