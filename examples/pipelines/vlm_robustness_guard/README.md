...
Three safety and robustness components for production VLM systems, run as a single evaluation pipeline.
...
All three components run end-to-end in under 10 seconds on CPU.
...
## Component 2: Per-sample OOD detection
...
- `run.py` — three components, `main()` entry point
- `../../../configs/vlm_robustness_guard.yaml` — thresholds, sample counts, injection rate
- `../../../src/production_vlm/robustness/` — the three underlying modules
